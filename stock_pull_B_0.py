...
class portfolio:
    def __init__(self, files):
        ...
        self.weed_out_by_size()

    def weed_out_by_size(self):
        file_size = {}
        for file in self.files:
            if '.csv' in file and 'combination' not in file:
                ...
                if 'last_file' in locals():
                    if len(reader) == file_size[last_file]:
                        file_size[file] = len(reader)
                    else:
                        if len(reader) > file_size[last_file]:
                            file_size.pop(last_file, None)
                            file_size[file] = len(reader)
                else:
                    file_size[file] = len(reader)
                

                print(file, len(reader))
